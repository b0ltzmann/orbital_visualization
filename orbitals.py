...
class Orbital:
    def __init__(self, l, ml, view, colormap='RdBu', steps=30, points=200):
        ...
        self.points = points
        self.theta, self.phi = np.meshgrid(np.linspace(0, np.pi, num=points),
                                           np.linspace(0, 2*np.pi, num=points))
        # Generate figure:
        self.fig    = plt.figure()
        if ml == 'all':
            self.fig.set_size_inches(12, 12)
            self.spec = gridspec.GridSpec(ncols=2*self.l+1,
                                          nrows=self.l+1,
                                          figure=self.fig)
        elif abs(ml) > l:
    ...
    def savegif(self, frames):
        ...
        firstframe = frames[0]
        firstframe.save('./animation.gif', format='GIF',
                        append_images=frames,
                        save_all=True, fps=10, loop=0)
            ...
            if self.ml == 'all':
                for l in range(self.l+1):
                    for ml in range(-l, l+1):
                        ax = self.fig.add_subplot(self.spec[l, ml+self.l],
                                                  projection='3d')
